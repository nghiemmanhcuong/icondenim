<div class="fixed">
    <div class="search">
        <div class="container search-container">
            ...
            <form class="search-form" method="get" action="<?=WEB_ROOT?>/search">
                <input class="search-form-input" type="text" placeholder="Tìm kiếm" name="keyword" >
                <button class="search-form-btn" type="submit">Tìm kiếm</button>
            </form>
            <div class="search-close" title="Đóng">
                <i class="fa-solid fa-xmark"></i>
            </div>
        </div>
    </div>
    <div class="popup-loyalty">
        <div class="popup-loyalty-icon">
            <i class="fa-solid fa-heart"></i>
        </div>
    </div>
    <div class="popup-loyalty-content">
        <div class="popup-loyalty-content-close" title="Đóng">
            <i class="fa-solid fa-circle-xmark"></i>
        </div>
        <?php if(isset($_COOKIE['user_id'])):?>
        <div class="popup-loyalty-content-user">
            <div class="popup-loyalty-content-user-avatar">
                <img src="<?=IMG_ROOT.$_SESSION['user_client']['avatar']?>" alt="">
            </div>
            <div class="popup-loyalty-content-user-info">
                <h5><?=$_SESSION['user_client']['name']?></h5>
                <p> <span><i class="fa-solid fa-crown"></i></span> 
                Member • <?=$_SESSION['user_client']['accumulated_points']?> điểm thưởng</p>
            </div>
        </div>
        <?php else:?>
        <div class="popup-loyalty-not-user">
            <div class="popup-loyalty-content-not-user-icon">
                <i class="fa-solid fa-heart"></i>
            </div>
            <h5>Chào bạn, cảm ơn bạn đã quan tâm đến shop icondenim.com. Chúng tôi có thể hỗ trợ gì cho bạn?</h5>
            <p>Hãy đăng nhập ngay để xem những ưu đãi hiện đang có của bạn</p>
            <a href="<?=WEB_ROOT?>/account/dang-nhap">Đăng nhập để tích điểm</a>
            <div class="popup-loyalty-not-user-register">Chưa có tài khoản?<a href="<?=WEB_ROOT?>/account/dang-ky">Đăng
                    ký ngay</a></div>
        </div>
        <?php endif;?>
    </div>
